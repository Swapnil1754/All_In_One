...
@Aspect
@Component
public class GenericAuditAspectImpl implements GenericAuditAspect {
    private static final Logger logger = LoggerFactory.getLogger(GenericAuditAspectImpl.class);
    @Around("@annotation(com.example.common_data.Annotations.DataAudit)")
    @Override
    public Object audit(ProceedingJoinPoint joinPoint) throws Throwable {
        // Before logic
        String request = getRequestJson(joinPoint);
        logger.info("GenericAuditAspect Before hitting the method: {}", request);
        postToElastic("Before", joinPoint);
        Object result;
        try {
            result = joinPoint.proceed();
        } catch (Throwable ex) {
            logger.error("GenericAuditAspect Exception in method: {}", ex.getMessage());
        ...
        // After logic
        logger.info("GenericAuditAspect After hitting the method: {}", joinPoint.getSignature());
